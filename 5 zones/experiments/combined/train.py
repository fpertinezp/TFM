...
def train_agent(seed):
    ...
    train_env = MultiObsWrapper(train_env, n=10, flatten=True)
    train_env = WeatherForecastingWrapper(train_env, n=5, delta=2, weather_variability=[1.0, 0.0, 0.001])
    train_env = NormalizeObservation(train_env)
    train_env = NormalizeAction(train_env)
    train_env = LoggerWrapper(train_env)

    eval_env = MultiObsWrapper(eval_env, n=10, flatten=True)
    eval_env = WeatherForecastingWrapper(eval_env, n=5, delta=2, weather_variability=[1.0, 0.0, 0.001])
